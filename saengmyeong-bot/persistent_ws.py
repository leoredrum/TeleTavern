...
async () => {
    try {
        ...
        extMod.extension_settings[KEY] = Object.assign({}, extMod.extension_settings[KEY] || {}, {
            ...
            preferredCharacterAvatar: '""" + TARGET_AVATAR + r"""',
            directorAvatars: [],
        });
        await scriptMod.saveSettings();
        await scriptMod.getCharacters();
        const idx = (scriptMod.characters || []).findIndex(c => c.avatar === '""" + TARGET_AVATAR + r"""');
        if (idx >= 0) {
...
async def main() -> int:
    ...
    async with async_playwright() as pw:
        ...
        # Phase 1: wait for ChatBridge WS to come online (up to 120s).
        print("[persistent_ws] phase 1: waiting for ChatBridge WS = 已连接 (up to 120s)", flush=True)
        ws_connected = False
        for i in range(240):
            try:
            ...
            if txt == "已连接":
                ws_connected = True
                print(f"[persistent_ws] ChatBridge WS 已连接 at t={i*0.5:.1f}s", flush=True)
                break
            await asyncio.sleep(0.5)

        if not ws_connected:
            print("[persistent_ws] WARN: ChatBridge never reached 已连接 in 120s, continuing anyway", flush=True)

        # Phase 2: force settings + tryActivate character (idempotent, survives ST frontend race).
        ...
        print("[persistent_ws] phase 3: idle (long-running); refresh settings every 5min", flush=True)

        # Phase 3: idle forever, re-inject settings every 5min to recover from frontend re-init.
