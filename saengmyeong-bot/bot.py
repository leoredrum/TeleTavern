...
This bot is CULT-RP, not RPG combat, not generic chat. It does NOT include the
DM Game State / RPG Rule / Director engines. The model is the world; the bot
just transports player actions, sessions, exports, and a Chinese Language
...
OPENAI_BASE_URL = os.environ.get("OPENAI_BASE_URL", "http://127.0.0.1:8022/v1")
...
def create_session(path: str, chat_id: int, title: str = "Saengmyeong Cult - new story",
                   character: str = DEFAULT_CHARACTER,
                   world_info: str = DEFAULT_WORLD_INFO) -> dict:
    path = _abs(path)
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        f.write("- Character Book: {world_info}\n\n".format(world_info=world_info))
        f.write("---\n\n")
    meta = {
...
4. **不**自动正能量化 cult 内容:
   ...
   - 让场景自然呈现 dystopia 反讽氛围
5. 直白描写身体 / 情感 / 仪式动作,**细节饱满不空泛**。

## 长度与节奏(LEO 强约束:文本长度太长切分)

1. 单次回复 **1000-2000 字** 为宜(cult RP 沉浸感更浓,需要细节描写)。
2. **超长回复(>4096 字符)由 telegram_splitter.py 自动切分**:
   - 第一段编辑 placeholder 流式预览
   - 后续段 reply_text 顺序发送
   - 段间保留语义连续性(不切散一个完整动作 / 对话 / 段落)
3. **语言层避免单段超长**:
   - 一个动作 / 一个对话回合 / 一个场景转换 = 单独段落
   - 不要把多个独立动作合并写成一个超长段落
...
async def _stream_chunks(user_text: str, history: list[dict] | None = None):
    ...
    messages = []
    if SAENGMYEONG_LANGUAGE_OVERRIDE:
...
# ============================================================================
HELP_TEXT = """📜 **SAENGMYEONG Bot** - 圣生 Cult-RP Bot
...
**自由文本**: 直接发剧情行动 → Bot 转发给 SillyTavern → SAENGMYEONG 流式回复

**角色卡**: Blessed Are The Fruitful (Saengmyeong-gyo / 생명교 / 神圣生命教)
**Character Book**: [Blessed Are The Fruitful] - Complete Lorebook (57 entries)
**输出**: 简体中文 + 第二人称 perspective + 韩文人名保留罗马音
...
async def cmd_start(update: Update, _ctx: ContextTypes.DEFAULT_TYPE) -> None:
    if not _update_dedup_check(update.effective_chat.id, update.update_id):
        await update.message.reply_text(
            ...
            "Character Book: 57 entries 自动激活\n"
            "设定: 2027 年首尔现代 dystopia / Saengmyeong-gyo cult-RP\n\n"
